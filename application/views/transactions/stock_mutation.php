<?php
	...
	$stockholder_id_to = (isset($form['stockholder_id_to'])?$form['stockholder_id_to']:null);
	$stockholder_name_to = (isset($form['stockholder_name_to'])?$form['stockholder_name_to']:null);
	$stockholder_cluster_to = (isset($form['stockholder_cluster_to'])?$form['stockholder_cluster_to']:null);
	...
	$stock_qty_to_before = (isset($form['qty_to_before'])?$form['qty_to_before']:0);
	$stock_qty_to_after = (isset($form['qty_to_after'])?$form['qty_to_after']:0);
...
?>
	...
	<div style="height:640px; width:700px; border: 1px solid darkgray; background-color: white; position:relative; top:25px; text-align:left; margin-left:auto; margin-right:auto;">
		...
		<h1>TRANSAKSI PENGALIHAN SAHAM</h1>
		<table border='0' cellpadding='0' cellspacing='15'><tbody><tr><td>
			...
			<table border="0" cellspacing="0" cellpadding="3"><tbody>
				...
				<tr><td colspan="3" height="25px"><strong>PEMEGANG SAHAM 1</strong></td></tr>
				<tr>
					<td>ID*</td>
					<td>:</td>
					<td>
				...
				<tr><td colspan="3" height="25px"><strong>PEMEGANG SAHAM 2</strong></td></tr>
				<tr>
					<td>ID*</td>
					<td>:</td>
					<td>
						<input type="text" id="stockholder_id_to" value="<?=$stockholder_id_to?>" style="border: none; font-size: 110%; width: 50px" placeholder="<?=$blank2;?>" readonly />
						<?php if($trans_status == 0) { ?>
						<a href="#" class="nounderline" onclick="stock_dialog_stockholder_search('Daftar Jamaah', 'to');" style="font-size: 90%">[...]</a>
						<?php } ?>
					</td>
				</tr>
				<tr>
					...
					<td>:</td>
					<td>
						<input type="text" id="stockholder_name_to" value="<?=$stockholder_name_to?>" style="border: none; font-size: 110%; width: 250px" placeholder="<?=$blank2;?>" readonly /> 
					</td>
				</tr>
				<tr>
					<td>Kelompok</td>
					<td>:</td>
					<td><input type="text" id="stockholder_cluster_to" value="<?=$stockholder_cluster_to?>" style="border: none; font-size: 110%; width: 150px" placeholder="<?=$blank2;?>" readonly /></td>
				</tr>
				<tr>
					<td>Jumlah Saham</td>
					<td>:</td>
					<td>
						Sebelum : <input type='text' id='stock_qty_to_before' value='<?=$stock_qty_to_before?>' style='border: none; font-size: 110%; width: 30px' readonly />
						Sesudah : <input type='text' id='stock_qty_to_after' value='<?=$stock_qty_to_after?>' style='border: none; font-size: 110%; width: 30px' readonly />
					</td>
				</tr>
...
<script type='text/javascript'>
	...
	function stock_mutation_detail_append(stock_no, total_price, trans_status, qty_flag) {
		...
		if(qty_flag == 0) { // setting qty hanya berlaku jika status transaksi masih open (0)
			...
			$('#stock_qty_to_after').val(Number($('#stock_qty_to_before').val()) + Number(stock_no));
		}		
		var str = "";
		if(stock_no % 2 == 1) {
		...
		if(trans_status == 0) {
		str +="	<a href='#' class='nounderline' style='font-size: 90%' onclick='stock_mutation_detail_remove("+stock_no+")'>[-]</a> ";
		}
		str += stock_no+".&nbsp;</td>"
	...
	function stock_mutation_detail_remove(stock_no) {
		...
		if(stock_qty == 1) { // jika stock tinggal 1 record
			...
			$('#stock_qty_to_after').val(Number($('#stock_qty_to_after').val()) - 1);
